...
### A-2. PostHog に取引金額を送っている件の判断 ⚠️
`src/services/analytics.ts:20-28` の `trackTransactionSaved` が **取引金額（`amount`）をそのまま
PostHog に送信**している。

影響:
- App Privacy ラベルで「**財務情報（Financial Info）**」を「分析」目的で収集と申告する必要が出る
- 家計簿アプリで金額を外部分析基盤に送るのは、ユーザーの期待とズレやすい

選択肢（**推奨: 案1**）:
1. **金額をレンジ化する**（例: `amountBucket: '1000-4999'`）。分布は分かるが個別金額は残らない。
   App Privacy の申告も軽くなる
2. `amount` を送信項目から削除する
3. そのまま送る → その場合、プライバシーポリシーの 3-5 を「金額そのものを送信」と明記し直し、
   App Privacy でも財務情報を申告する

- [ ] 方針を決める
- [ ] コードを修正する（案1/2の場合）
- [ ] プライバシーポリシー 3-5 の記述を最終形に合わせる
